...
namespace PicklesDoc.Pickles.CommandLine
{
    ...
    {
        private static readonly Logger Log = LogManager.GetLogger(""); // TODO: MethodBase.GetCurrentMethod().DeclaringType.Name);

        private static void Main(string[] args)
        {
            var builder = new ContainerBuilder();
            builder.RegisterAssemblyTypes(typeof(Runner).GetTypeInfo().Assembly);
            ...
            if (shouldContinue)
            {
                if (Log.IsInfoEnabled)
                {
                    Log.Info(
                        "Pickles v.{0}{1}",
                        typeof(Program).GetTypeInfo().Assembly.GetName().Version,
                        Environment.NewLine);
